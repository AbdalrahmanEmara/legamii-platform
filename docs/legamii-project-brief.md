...
│   ├── grade.service.js         ⬜ not started
│   └── cloudinary.service.js    ⬜ not started
├── actions/
│   ├── auth.actions.js          ✅
│   ├── quiz.actions.js          ✅
│   └── contest.actions.js       ⬜ not started
├── validators/
│   └── auth.schema.js           ✅ (all auth schemas)
├── constants.js                 ✅
└── utils.js                     ⬜ not started
```
...
| `/practice` | ✅ |
| `/practice/[quiz_id]` | ✅ |
| `/contests` | ✅ (UI done, needs contest.actions) |
| `/contests/[id]` | ⬜ not started |
| `/contests/[id]/play` | ⬜ not started |
| `/contests/[id]/summary` | ⬜ not started |
...
## Next Steps
1. `/auth/academic` — grade + term onboarding page
2. `contest.actions.js` — all contest actions
3. Contest pages — lobby, play, summary
4. `services/index.js` — re-export all services
5. `middleware.js` — protect routes, redirect to `/auth/signin` if no token
