...
export default function Streak({ days = 14, bestStreak = 30, totalDays = 100 }) {
  return (
      ...
      <div className="flex flex-col gap-base p-base">
        ...
        <div className="flex flex-col items-center gap-sm">
          ...
          <div className="flex flex-col items-center gap-xs2">
            <span className="text-neutral-950 text-base font-normal font-primary leading-6 md:text-xl 2xl:text-2xl 2xl:leading-8">{days}</span>
          ...
          <div className="w-full flex items-center justify-center gap-2 flex-wrap">
            {daysOfTheWeek.map((day, index) => (
              <div key={index} className={index < daysOfTheWeek.length - 3 ? solvedClass : missedClass}>
                {day}
              </div>
            ))}
          </div>
          ...
          <div className="w-full flex items-center justify-center gap-3 flex-wrap">
            <div className="w-full bg-neutral-50 border border-neutral-200 text-text p-sm flex justify-between font-normal leading-4">
              ...
              <span className="text-caption-1 2xl:text-sm font-primary">{bestStreak}</span>
            </div>
            <div className="w-full bg-neutral-50 border border-neutral-200 text-text p-sm flex justify-between font-normal leading-4">
              <span className="text-xs 2xl:text-sm font-secondary">Total Days</span>
              <span className="text-caption-1 2xl:text-sm font-primary">{totalDays}</span>
            </div>
          </div>
        </div>
