...
export default function Leaderboard(){
  return(
    <ReusableWindow className="flex w-[413px] h-[1004px] flex-col items-start items-stretch" title="Global_Leaderboard.sys"
    >
      <CustomScroll>
        {leaderboardData.map((user, index) => {
          ...
          const isYou = user.name === "YOU";
           ...
           return(
             <div key={user.id} className={`flex px-sm py-base gap-base self-stretch ${
                isTopOne
...
{/* Rank */}
              <div className="w-10 label-1 font-bold">#{user.id}</div>

                    {/* Avatar */}
              <div className="relative h-[50.304px] w-[50.304px] border-border self-stretch">
                <Image
                  src={user.avatar}
                  alt={user.name}
                  fill
                ...
                />
                          ...
                          {/* Name + Level */}
              <div className="flex flex-col justify-center items-start gap-xxs">
                <span
                  className="label-1 font-bold"
                >
                  {user.name}
                </span>
       ...
       {/* ================= FOOTER ================= */}
      <div className="flex bg-black p-sm font-bold justify-between items-center self-stretch border-border ">
        <span className="font-bold text-neutral-50 label-1">Your Rank</span>
        <span className="text-primary-300 label-1 font-bold"># {leaderboardData.findIndex(user => user.name === "YOU") + 1}</span>
